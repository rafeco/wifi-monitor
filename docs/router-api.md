...
- **Compatibility probe.** This integration is ASUS/ASUSWRT-only. When viewing the current network in Settings, `RouterService.probeCompatibility(host:)` makes a lightweight HTTP request to the gateway and looks for ASUS markers (an "asus" string on the landing page, or a 200 from `/Main_Login.asp`). The result (`supported` / `unsupported` / `unknown` when unreachable) is stored on the `NetworkProfile`. The connect flow is only offered for `supported` routers; `unsupported` shows an explanation with a "Configure anyway" manual override. The probe is best-effort and heuristic.
