...
- **ISP detection is gated on the external IP** — Cloudflare's `https://1.1.1.1/cdn-cgi/trace` (unmetered, 192 bytes) reports the external IP every 2 minutes; an ipinfo.io org lookup is spent only when that IP changes, when there's no cached answer, or every 6 hours as a safety net. Unauthenticated ipinfo lookups are capped per source IP per day, so the org is cached across launches (keyed by SSID), failures back off exponentially to 2 hours, and a floor between lookups bounds the worst case. If the IP probe itself fails, detection falls back to a 15-minute ipinfo poll. Responses are validated (`isPlausibleConnectionName`) because a rate-limited request answers with a JSON error body, not an org string.
